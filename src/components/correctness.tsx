// src/components/CorrectnessFloater.tsx
import React, { useState, useEffect } from 'react';
import { Trash2, Edit2, Plus, X, CheckCircle } from 'lucide-react';

export interface CorrectnessEntry {
  id?: string;
  text: string;
  created_at?: string;
}

// CHANGE THIS to your deployed worker URL in production
// Make sure this matches the route where you deployed the backend above

  const BASE_URL = import.meta.env.PUBLIC_BACKEND_URL;
  const API_URL = `${BASE_URL}/correctness`; 
const CorrectnessFloater = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [entries, setEntries] = useState<CorrectnessEntry[]>([]);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Tear this page out?')) return;
    try {
  ...
  const handleEdit = (entry: CorrectnessEntry) => {
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 left-20 bg-black text-white p-4 rounded-full shadow-2xl hover:scale-110 transition-transform z-50 flex items-center justify-center border-2 border-white"
      >
        <CheckCircle size={24} />
      </button>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-md rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            ...
            <div className="bg-black text-white p-4 flex justify-between items-center shrink-0">
              <span className="font-bold text-lg tracking-wider uppercase">The Correctness Diary</span>
            ...
            <div className="bg-gray-50 border-b shrink-0 transition-all duration-300">
              {!showInput ? (
                ...
                >
                  <Plus size={20} />
                  <span className="font-medium text-sm uppercase tracking-wide">Add new win</span>
                </button>
              ) : (
                // STATE 2: The Form
                <div className="p-4 animate-in slide-in-from-top-2 fade-in duration-200">
                  <form onSubmit={handleSubmit} className="flex flex-col gap-2">
                    <textarea
                      ...
                      rows={3}
                      placeholder={editingId ? "Refine the entry..." : "Write it down to remember it..."}
                      value={inputText}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-gray-100">
              ...
              {!loading && entries.map((entry) => (
                <div key={entry.id} className="group bg-white p-3 rounded shadow-sm border border-gray-100 flex justify-between items-start hover:shadow-md transition-shadow">
                  <p className="text-gray-800 text-sm whitespace-pre-wrap leading-relaxed">{entry.text}</p>
...
export default CorrectnessFloater;